...
function header(t){document.body.classList.remove("home-mode");step.textContent=t}
...
function intro(){
 ...
 screen.innerHTML=`<section class="hero-home" aria-label="Aventura de Idiomas">
   <div class="hero-art"></div>
   <div class="hero-shimmer"></div>

   <div class="hero-buddy" id="heroBuddy">
     <img src="robot_mascot.png" alt="Robot compañero">
     <div class="buddy-sparkles">✦ ✨ ✦</div>
   </div>

   <div class="hero-greeting" id="heroGreeting">
     <strong>¡Hola!</strong><br>¿Cómo te llamas?
   </div>

   <div class="hero-interaction">
     <div class="hero-card">
       <input class="hero-input" id="homeName" placeholder="Escribe tu nombre" autocomplete="name">
       <button class="hero-continue" id="homeContinue">Continuar →</button>
       <button class="hero-mic" id="homeMic">🎙️ Decírmelo</button>
       <div class="hero-note">La voz es opcional si el navegador no la permite.</div>
     </div>
   </div>
   <div class="hero-status" id="heroStatus">✨ ¡Un mundo de aventuras te espera!</div>
 </section>`;

 const input=document.getElementById("homeName");
 const cont=document.getElementById("homeContinue");
 const mic=document.getElementById("homeMic");
 const status=document.getElementById("heroStatus");
 const buddy=document.getElementById("heroBuddy");
 ...
 const startAdventure=()=>{
   ...
   if(!v){
     setStatus("👋 Primero dime tu nombre");
   ...
   state.name=v;save();
   cont.disabled=true;mic.disabled=true;
   setStatus(`🎉 ¡Hola, ${state.name}!`);
   setBuddyState("celebrate");
   speak(`¡Guau, ${state.name}! ¡Qué alegría conocerte! Vamos a comenzar nuestra aventura.`)
     .then(()=>ageScreen());
 };

 cont.onclick=startAdventure;

 mic.onclick=()=>{
   if(speaking)return;
   setStatus("🎙️ Activando micrófono…");
   setBuddyState("listening");
   const ok=listen(t=>{
     state.name=t.trim();save();input.value=state.name;
     setStatus(`🎉 ¡Hola, ${state.name}!`);
     setBuddyState("celebrate");
     speak(`¡Guau, ${state.name}! ¡Qué alegría conocerte!`)
       .then(()=>ageScreen());
   },"es-MX",{
     onFail:()=>{
       setBuddyState("idle");
       setStatus("🎙️ No te escuché. Puedes intentarlo otra vez o escribir tu nombre.");
     }
   });
   if(!ok){
     setBuddyState("idle");
     setStatus("🎙️ Tu navegador no permite usar el micrófono aquí. Puedes escribir tu nombre.");
   }
 };
 ...
 setTimeout(()=>{
   if(!speaking){
     ...
     speak("¡Hola! Soy tu amiguito. ¡Qué emoción! Vamos a explorar un mundo lleno de aventuras y aprender idiomas juntos. ¿Cómo te llamas?");
   }
 },350);
}
