...
function speak(text,lang="es-MX",rate=.88){
 ...
 speechQueue=speechQueue.then(()=>new Promise(resolve=>{
   speaking=true;talking.classList.add("on");
   if(!("speechSynthesis"in window)){speaking=false;talking.classList.remove("on");resolve();return}
   speechSynthesis.cancel();
   let finished=false;
   const finish=()=>{if(finished)return;finished=true;if(myToken===speechToken){speaking=false;talking.classList.remove("on")}resolve()};
